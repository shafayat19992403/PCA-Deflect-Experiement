...
class Helper:
    ...
    def model_dist_norm_var(model, target_params_variables, norm=2):
        ...
        sum_var = torch.zeros(size, dtype=torch.float32, device=device)
        size = 0
        for name, layer in model.named_parameters():
    ...
    def average_shrink_models(self, weight_accumulator, target_model, epoch):
        ...
        """
        # malicious_clients, most_important_weights = guardfl.apply_pca_to_weights(weight_accumulator.values(),weight_accumulator.keys(),epoch, None)
        ...
        for name, data in target_model.state_dict().items():
            ...
            data = data.float()  # Ensure data is a float tensor
