...
import logging
import torch
import attacks.CerP.train as train
import attacks.CerP.test as test
from attacks.CerP.cifar100_helper import Cifar100Helper
from attacks.CerP.utils.utils import dict_html
import attacks.CerP.utils.csv_record as csv_record
import yaml
import time
import visdom
import numpy as np
...
# vis = visdom.Visdom(port=8098, use_incoming_socket=False)
vis = None
...
def main_cerp(defense_name, dataset_name):
    ...
    with open('./attacks/CerP/utils/mkrum.yaml', 'r') as f:
    ...
    # load trigger
    if params_loaded['type'] == config.TYPE_CIFAR100 or params_loaded['type'] == config.TYPE_CIFAR:
    ...
    elif params_loaded['type']==config.TYPE_MNIST or params_loaded['type']==config.TYPE_FMNIST or params_loaded['type']==config.TYPE_EMNIST:
        data_iterator = helper.test_data
        triggervalue = 1
        for batch_id, (datas, labels) in enumerate(data_iterator):
            x = Variable(cuda(datas, True))
    ...
    for epoch in range(helper.start_epoch, helper.params['epochs'] + 1, helper.params['aggr_epoch_interval']):
        ...
        if helper.params["aggregation_methods"] == config.AGGR_PCA_DEFLECT:
            ...
            client_weights_flat = pca_deflect.extract_client_weights(client_weights)
            outliers, _ = pca_deflect.apply_pca_to_weights(client_weights_flat, client_names, epoch, [])
        ...
        weight_accumulator, updates = helper.accumulate_weight(weight_accumulator, epochs_submit_update_dict, agent_name_keys, num_samples_dict)
